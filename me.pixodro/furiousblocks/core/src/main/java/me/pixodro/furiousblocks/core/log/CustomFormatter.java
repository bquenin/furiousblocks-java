package me.pixodro.furiousblocks.core.log;
...
/**
 * A {@link Formatter} that may be customised in a {@code logging.properties} file. The syntax of the property
 * {@code com.thinktankmaths.logging.TerseFormatter.format} specifies the output. A newline will be appended to the string and the
...
public class CustomFormatter extends Formatter {
  ...
  public CustomFormatter() {
    ...
    format = format.replace("%L", "{0}").replace("%m", "{1}").replace("%M", "{2}").replace("%t", "{3}").replace("%c", "{4}").replace("%T", "{5}").replace("%n", "{6}").replace("%C", "{7}") + "\n";
  ...
  public String format(final LogRecord record) {
    final String[] arguments = new String[8];
    // %L
